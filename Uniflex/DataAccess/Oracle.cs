...
namespace I_HUB.DataAccess
{
    public class Oracle : AbsractOracle
    {
        ...
        public Oracle()
        {
            var connectionStringEncrypted = "";
            var OraDev = Helper.ConfigurationFactory.GetConfiguration("ConnectionStrings", "Oracle.Dev");// etrinadev
            var OraProd = Helper.ConfigurationFactory.GetConfiguration("ConnectionStrings", "Oracle.Prod");
            
            if ((!string.IsNullOrEmpty(OraDev)) && !string.IsNullOrEmpty(OraProd))
            {
                connectionStringEncrypted = new Helper.clsCryptho().Decrypt(OraProd, "@Pelindo");
            }
            else if ((string.IsNullOrEmpty(OraDev)) && !string.IsNullOrEmpty(OraProd))
            {
                connectionStringEncrypted = new Helper.clsCryptho().Decrypt(OraProd, "@Pelindo");
            }else if ((!string.IsNullOrEmpty(OraDev)) && string.IsNullOrEmpty(OraProd))
            {
                connectionStringEncrypted = new Helper.clsCryptho().Decrypt(OraDev, "@Pelindo");
            }

            //System.Diagnostics.Debug.WriteLine(connectionStringEncrypted);
           
            DbConnection = new OracleConnection(connectionStringEncrypted);
