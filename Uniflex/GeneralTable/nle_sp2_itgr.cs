...
namespace ILCS.GeneralTable
{
    public class nle_sp2_itgr_HEADER
    {
        ...
        public static List<Microsoft.AspNetCore.Mvc.Rendering.SelectListItem> getTerminalForDatasource()
        {
            ...
            using (I_HUB.DataAccess.Oracle db = new I_HUB.DataAccess.Oracle())
            {
                ...
                using (Oracle.ManagedDataAccess.Client.OracleDataReader dr = db.DbDataReader as Oracle.ManagedDataAccess.Client.OracleDataReader)
                {
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            ...
                            var valnya = I_HUB.Helper.GeneralHelper.NullToString(dr["KD_TERMINAL"]);
                            var textNya = I_HUB.Helper.GeneralHelper.NullToString(dr["TERMINAL"]);
                            item.Value = valnya;
            ...
            using (I_HUB.DataAccess.Oracle db = new I_HUB.DataAccess.Oracle())
            {
                ...
                using (Oracle.ManagedDataAccess.Client.OracleDataReader dr = db.DbDataReader as Oracle.ManagedDataAccess.Client.OracleDataReader)
                {
                    if (dr.HasRows)
                    {
                        ...
                        {
                            Microsoft.AspNetCore.Mvc.Rendering.SelectListItem item = new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem();
                            var v_ =I_HUB.Helper.GeneralHelper.NullToString(dr["BL_NO"]);
                            var t_ = I_HUB.Helper.GeneralHelper.NullToString(dr["NM_CARGOOWNER"]);
                            item.Value = v_;
                            item.Text = v_ + " - " + t_;
                            groups.Add(item);
                        }
                ...
                using (Oracle.ManagedDataAccess.Client.OracleDataReader rdr = db.DbDataReader as Oracle.ManagedDataAccess.Client.OracleDataReader)
                {
                    if (rdr.HasRows)
                    {
                        while (rdr.Read())
                        {
                            ...
                            {
                                CREATED_DATE =I_HUB.Helper.GeneralHelper.NullToDateTime(rdr["CREATED_DATE_"], System.DateTime.Now),
                                BL_NO = I_HUB.Helper.GeneralHelper.NullToString(rdr["BL_NO"]),
                                MESSAGE_SENT = I_HUB.Helper.GeneralHelper.NullToString(rdr["MESSAGE_SENT"]),
                                NM_CARGOOWNER = I_HUB.Helper.GeneralHelper.NullToString(rdr["NM_CARGOOWNER"]),
                                PRICE = I_HUB.Helper.GeneralHelper.NullToDecimal(rdr["PRICE"], 0),
                                SEND_DATE = I_HUB.Helper.GeneralHelper.NullToDateTime(rdr["SEND_DATE_"], System.DateTime.Now),
                                STATUS_SENT = I_HUB.Helper.GeneralHelper.NullToString(rdr["STATUS_SENT"]),
                                TERMINAL = I_HUB.Helper.GeneralHelper.NullToString(rdr["TERMINAL"])
                            };
                            l.Add(sp2);
                        }
                    }
        ...
        public static int GetCount(int mont, int yer, string status_sent)
        {
            ...
            using (I_HUB.DataAccess.Oracle db = new I_HUB.DataAccess.Oracle())
            {
                ...
                using (Oracle.ManagedDataAccess.Client.OracleDataReader rdr = db.DbDataReader as Oracle.ManagedDataAccess.Client.OracleDataReader)
                {
                    ...
                    {
                        rdr.Read();
                        return_ =  I_HUB.Helper.GeneralHelper.NullToInt(rdr["COUNT"], 0);
                    }
                    if (!rdr.IsClosed) rdr.Close();
                }
                db.Close();
