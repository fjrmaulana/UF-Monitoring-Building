...
namespace I_HUB.Helper
{
    public static class GeneralHelper
    {
        ...
        public static System.DateTime StringToDate(string dateNya, string format_)
        {
            System.DateTime d = System.DateTime.Now;
            if (string.IsNullOrEmpty(dateNya))
            {
                d = System.DateTime.Now;
            }
            else
            {
                d = System.DateTime.ParseExact(dateNya, format_, null);
            }
            return d;
        }
    ...
    public static class EnumExtensions
    {
        public static Microsoft.AspNetCore.Mvc.Rendering.SelectList ToSelectList<T>(this T enumObj) where T : struct, IComparable, IFormattable, IConvertible
        {
            ...
            return new Microsoft.AspNetCore.Mvc.Rendering.SelectList(values, "Id", "Name", enumObj);
        }

        public static Microsoft.AspNetCore.Mvc.Rendering.SelectList ToSelectList<T>(this T enumObj, bool useDescription, string selectedValue) where T : struct, IComparable, IFormattable, IConvertible
        {
            if (useDescription)
            {
                var values = from T e in Enum.GetValues(typeof(T)).Cast<Enum>()
                             select new Microsoft.AspNetCore.Mvc.Rendering.SelectListItem() { Value = Convert.ToInt32(e).ToString(), Text = (Attribute.GetCustomAttribute(e.GetType().GetField(e.ToString()), typeof(System.ComponentModel.DescriptionAttribute)) as System.ComponentModel.DescriptionAttribute).Description };
                return new Microsoft.AspNetCore.Mvc.Rendering.SelectList(values, "Value", "Text", selectedValue);
            }
            else
            {
                var values = from T e in Enum.GetValues(typeof(T)).Cast<Enum>()
                ...
                if ((object)obj is Enum)
                {
                    ...
                    foreach (int num in Enum.GetValues(type))
                    {
                        if (num == Convert.ToInt32(obj, System.Globalization.CultureInfo.InvariantCulture))
                        {
                            ...
                            {
                                result = ((System.ComponentModel.DescriptionAttribute)customAttributes[0]).Description;
                            }
